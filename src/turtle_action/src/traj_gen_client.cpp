...
{
public:
  using TrajGen = turtle_action_interface::action::Fibonacci;
  using GoalHandleTrajGen = rclcpp_action::ClientGoalHandle<TrajGen>;

  explicit FibonacciActionClient(const rclcpp::NodeOptions & options)
  : Node("turtle_action_client", options)
  {
    this->client_ptr_ = rclcpp_action::create_client<TrajGen>(
      this,
      "fibonacci");
  ...
  void send_goal()
  {
    ...
    auto goal_msg = TrajGen::Goal();
    goal_msg.order = 10;

    RCLCPP_INFO(this->get_logger(), "Sending goal");

    auto send_goal_options = rclcpp_action::Client<TrajGen>::SendGoalOptions();
